...
def customerAction(connection, custID):
    ...
    while customerInteracting == True:
        ...
        if leInput == "back":
            ...
            cursor.execute( SQL )
            print("{0:10}{1:10}{2:12}{3:20}{4:10}{5:32}".format("Invoice ID", "Line ID", "Clothing ID", "Name", "Sold For", "Credit Card"))
            for (InvoiceID, InvoiceLineId, ClothingId, Name, SoldFor, Credit) in cursor:
                print("{0:10}{1:10}{2:12}{3:20}${4:10}{5:32}".format(str(InvoiceID), str(InvoiceLineId), str(ClothingId), Name, str(SoldFor), Credit))
            cursor.close()
        elif leInput == "1":
            cursor = connection.cursor()
